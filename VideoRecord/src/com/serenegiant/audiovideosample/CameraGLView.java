...
/**
 * Sub class of GLSurfaceView to display camera preview and write video frame to
 * capturing surface
 */
public final class CameraGLView extends GLSurfaceView {
	...
	public void setMediaMuxerWrapper(MediaMuxerWrapper tMediaMuxerWrapper) {
		mMediaMuxerWrapper = tMediaMuxerWrapper;
	}
	...
	public void surfaceDestroyed(final SurfaceHolder holder) {
		...
		if (mCameraHandler != null) {
			...
			// otherwise camera try to display on un-exist Surface and some
			// error will occure
	...
	private static final class CameraSurfaceRenderer
			implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener { // API
																							// >=
																							// 11
		...
		public void onSurfaceCreated(final GL10 unused, final EGLConfig config) {
			...
			final String extensions = GLES20.glGetString(GLES20.GL_EXTENSIONS); // API
																				// >=
																				// 8
			// if (DEBUG) Log.i(TAG, "onSurfaceCreated:Gl extensions: " +
			// extensions);
			...
			// clear screen with yellow color so that you can see rendering
			// rectangle
		...
		public void onSurfaceChanged(final GL10 unused, final int width, final int height) {
			...
			// if at least with or height is zero, initialization of this view
			// is still progress.
			if ((width == 0) || (height == 0))
				return;
			updateViewport();
		...
		private final void updateViewport() {
			...
			if (parent != null) {
				...
				Log.i(TAG, String.format("view(%d,%d)%f,video(%1.0f,%1.0f)", view_width, view_height, view_aspect,
				...
				case SCALE_KEEP_ASPECT_VIEWPORT: {
					...
					int width, height;
					if (view_aspect > req) {
						// if view is wider than camera image, calc width of
						// drawing area based on view height
						y = 0;
						height = view_height;
						width = (int) (req * view_height);
						x = (view_width - width) / 2;
					} else {
						// if view is higher than camera image, calc height of
						// drawing area based on view width
						x = 0;
						width = view_width;
						height = (int) (view_width / req);
						y = (view_height - height) / 2;
					}
					// set viewport to draw keeping aspect ration of camera
					// image
					if (DEBUG)
				...
				case SCALE_CROP_CENTER: {
					...
					final double scale = (parent.mScaleMode == SCALE_CROP_CENTER ? Math.max(scale_x, scale_y)
							: Math.min(scale_x, scale_y));
					final double width = scale * video_width;
					final double height = scale * video_height;
					Log.v(TAG, String.format("size(%1.0f,%1.0f),scale(%f,%f),mat(%f,%f)", width, height, scale_x,
		...
		/**
		 * drawing to GLSurface we set renderMode to
		 * GLSurfaceView.RENDERMODE_WHEN_DIRTY, this method is only called when
		 * #requestRender is called(= when texture is required to update) if you
		 * don't set RENDERMODE_WHEN_DIRTY, this method is called at maximum
		 * 60fps
		 */
		@Override
			...
			if (flip) { // ~30fps
				synchronized (this) {
					if (mVideoEncoder != null) {
						// notify to capturing thread that the camera frame is
						// available.
	...
	private static final class CameraHandler extends Handler {
		...
		private static final int MSG_TAKE = 3;
		...
		public void takephoto() {
			sendMessage(obtainMessage(MSG_TAKE));
		}
		 ...
		 * 
		 * @param needWait
		 *            need to wait for stopping camera preview
		 */
	...
	private final class CameraThread extends Thread {
		...
		public void takePhoto() {
			...
			mCamera.takePicture(null, null, new PictureCallback() {
				...
				public void onPictureTaken(byte[] data, Camera camera) {
					File fileFolder = new File(mMediaMuxerWrapper.getOutputNamePath() + ".jpg");
					FileOutputStream outputStream;
					try {
						Bitmap bMap;
						bMap = BitmapFactory.decodeByteArray(data, 0, data.length, null);
						int orientation = 0;
						...
						if (orientation != 0) {
							...
							bMapRotate = Bitmap.createBitmap(bMap, 0, 0, bMap.getWidth(), bMap.getHeight(), matrix,
									true);
						} else
							bMapRotate = Bitmap.createScaledBitmap(bMap, bMap.getWidth(), bMap.getHeight(), true);

						try {
							outputStream = new FileOutputStream(fileFolder);
		...
		private final void startPreview(final int width, final int height) {
			...
			if ((parent != null) && (mCamera == null)) {
				...
				try {
					...
					// let's try fastest frame rate. You will get near 60fps,
					// but your device become hot.
					final List<int[]> supportedFpsRange = params.getSupportedPreviewFpsRange();
					// final int n = supportedFpsRange != null ?
					// supportedFpsRange.size() : 0;
					// int[] range;
					// for (int i = 0; i < n; i++) {
					// range = supportedFpsRange.get(i);
					// Log.i(TAG, String.format("supportedFpsRange(%d)=(%d,%d)",
					// i, range[0], range[1]));
					// }
					...
					final Camera.Size closestSize = getClosestSupportedSize(params.getSupportedPreviewSizes(), width,
							height);
					params.setPreviewSize(closestSize.width, closestSize.height);
					// request closest picture size for an aspect ratio issue on
					// Nexus7
					final Camera.Size pictureSize = getClosestSupportedSize(params.getSupportedPictureSizes(), width,
							height);
					...
					// adjust view size with keeping the aspect ration of camera
					// preview.
					// here is not a UI thread and we should request parent view
					// to execute.
		...
		private Camera.Size getClosestSupportedSize(List<Camera.Size> supportedSizes, final int requestedWidth,
		...
		private final void setRotation(final Camera.Parameters params) {
			...
			final Display display = ((WindowManager) parent.getContext().getSystemService(Context.WINDOW_SERVICE))
					.getDefaultDisplay();
			...
			// XXX This method fails to call and camera stops working on some
			// devices.
